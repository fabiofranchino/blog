...
The rules behind that dark piece of string could be outlined as the follows:

- An expression starts always with the reserved char **/**
- Find any word that begins with **http**
- Optionally includes words with or without the letter **s** after the first pattern rule (resulting with http or https) using the special char **?**
- Continue to find words that include **//** (since the / is a reserved char we need to escape them with the back-slash, therefore, \/\/ means double slash)
- Finally, continue to select words that include any alpha-numeric characters other than spaces (with the specific command **\S**) and continue until the condition won't be met anymore (using ***** char)
- The **/g** closure means **recursive**, find every occurrence.
